...
def main():
    ...
    for x in test_generator:
        ...
        prediction = siamese_model.predict([x], verbose=0)
        if i == 0:
    ...
    with open(args.prediction, 'w') as outFile:
        outFile.write('qid-a\tqid-b\tpred\tclass\n')
        i = 0
    ...
    predict_list = []
    total_test_samples = predictions.shape[0]
    ...
    ground_truth = gt_file[:, 2:]

    score = accuracy_score(ground_truth, predict_list)
    print("Accuracy gained on testset: {}".format(round(score, 4)))
    #accuracy_score_list.append(score)

    score_aggregate(args.prediction, args.qpp_file_path)
